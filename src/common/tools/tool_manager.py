...
class ToolManager:
    ...
    def _discover_tools(self) -> None:
        ...
        for file_path in tool_dir.glob("*.py"):
            ...
            try:
                # Import the module
                module = importlib.import_module(f".{module_name}", package="src.common.tools")
                ...
                self.logger.warning(f"Failed to import tool module {module_name}: {e}")
    ...
    def _register_tool(self, tool_class: Type[YOPOBaseTool]) -> None:
        ...
        try:
            ...
            self.logger.info(f"Initialized tool base: {name}")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize tool base '{tool_class.__name__}': {e}")
