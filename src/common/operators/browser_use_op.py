...
async def execute_browser_task_node(state: BrowserUseState, model: str, custom_sys_prompt: Optional[str] = None) -> BrowserUseState:
    """
    ...
    try:
        if context:
            agent = BrowserAgent(
                ...
                extend_system_message=f"Context: {context}" if not custom_sys_prompt else custom_sys_prompt + f"Context: {context}"
            )
        else:
            agent = BrowserAgent(
                task=original_query,
                ...
                browser_profile=browser_profile,
                page_extraction_llm=llm,
                extend_system_message="" if not custom_sys_prompt else custom_sys_prompt
            )
...
class BrowserUseOperator(BaseOperator):
    ...
    def __init__(self, 
        ...
        super().__init__(name=name, description=description, model_name=os.getenv("BROWSER_USER_MODEL_NAME", "gpt-4.1-mini"), **kwargs)
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        async def _execute_task_wrapper(state: BrowserUseState) -> BrowserUseState:
            return await execute_browser_task_node(state, self.model_name, self.system_prompt)
    ...
    async def arun(self, query: str, context: Optional[Dict[str, Any]] = None) -> OperatorResult:
        ...
        """
        logger.info(f"🎯 Starting browser automation for query: {query}, context: {context}")
        ...
        try:
            ...
            logger.info(f"{self.name} Operator result: {final_state['task_result']}")
            # Prepare result
