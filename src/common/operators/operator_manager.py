...
import os
from typing import Dict, Any, Optional, List, Type, get_type_hints, Tuple
import inspect
import importlib
...
class OperatorInfo(BaseModel):
    ...
    arun_doc: str = Field(description="Documentation for the arun method")
...
class OperatorManager:
    ...
    def _register_operator(self, operator_class: Type[BaseOperator], module_name: str, prompt_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            arun_schema, arun_doc = self._extract_arun_schema(operator_class)
    ...
    def _extract_arun_schema(self, operator_class: Type[BaseOperator]) -> Tuple[Dict[str, Any], str]:
        ...
        try:
            ...
            # Add docstring to schema
            arun_doc: str = arun_method.__doc__ or ""
            
            return schema, arun_doc
...
if __name__ == "__main__":
    ...
    async def test_file_processing_operator():
        ...
        # Example test with project's test data
        from pathlib import Path
        project_root = Path(__file__).parent.parent.parent.parent
        test_file = project_root / "data" / "benchmark_data" / "gaia" / "validation" / "65afbc8a-89ca-4ad5-8d62-355bb401f61d.xlsx"
        result = await file_op.arun(query="What are the file about?", sources=[str(test_file)])
    ...
    async def test_format_operator():
        """Simple test for BrowserUseOperator."""
        print("Testing Format...")
        
        browser_op = FormatOperator(prompt_path="./prompts/format_prompts.yaml")
        
        # result = await browser_op.arun(input_data="The red numbers are 1,2,3,7,223,5;The green numbers are 33,5,1,2", target_type='dict', format_schema="\{'red': list, 'green': list\}")
        result = await browser_op.arun(input_data="The red numbers is 1,2,3,4", target_type='list')
        print(f"Result: {result.base_result}")
    
    async def test_deep_research_operator():
        """Simple test for BrowserUseOperator."""
        print("Testing DeepRe...")
        
        dr_op = DeepResearchOperator(prompt_path="./prompts/deep_research_prompts.yaml")
        
        # result = await browser_op.arun(input_data="The red numbers are 1,2,3,7,223,5;The green numbers are 33,5,1,2", target_type='dict', format_schema="\{'red': list, 'green': list\}")
        result = await dr_op.arun(query="In the Scikit-Learn July 2017 changelog, what other predictor base command received a bug fix? Just give the name, not a path.")
        # result = await dr_op.arun(query="when is Donal Trump's birthday?")
        print(f"Result: {result.base_result}")
    

    # Run the test
    asyncio.run(test_deep_research_operator()) 
