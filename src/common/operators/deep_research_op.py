...
async def optimize_query_node(state: ResearchState, prompts: Dict[str, str]) -> ResearchState:
    ...
    logger.info("Starting query optimization node")
    ...
    try:
        ...
        for item in result.get("optimized_queries", []):
            state["optimized_queries"].append(item.get("optimized_query", []))
            state["key_concepts"].extend(result.get("key_concepts", []))
        if state["optimized_queries"] == []:
            state["optimized_queries"] = [original_query]
        logger.info(f"Optimized queries: {state['optimized_queries']}")
        logger.info(f"Key concepts: {state['key_concepts']}")

    except json.JSONDecodeError:
        logger.warning("Failed to parse JSON response, using fallback values")
        # Fallback if JSON parsing fails
        state["optimized_queries"] = [state["original_query"]]
        state["key_concepts"] = []
    
    state["messages"] = messages + [AIMessage(content=response)]
    logger.info("Query optimization node completed")
    return state

async def execute_search_node(state: ResearchState, prompts: Dict[str, str], custom_sys_prompt: Optional[str] = None) -> ResearchState:
    """Node 2: Execute searches using fire_crawl and gather information."""
    prompt = prompts.get('search_executor_prompt', '') if not custom_sys_prompt else custom_sys_prompt
    ...
    try:
        ...
        logger.warning(f"Could not get MCP tools: {str(e)}")
        tools = []
    ...
    async def execute_single_search(query: str) -> Dict[str, Any]:
        ...
        if not query:
            logger.warning("Received empty query for search")
            return {
                "query_used": query,
                "error": "Empty query"
            }
    ...
    if state["followup_questions"] == []:
        optimized_queries: List[str] = state["optimized_queries"]
        query_list = optimized_queries
        logger.info(f"Using optimized query")
    else:
        query_list = state["followup_questions"]
        logger.info(f"Using {len(query_list)} follow-up questions for search")
    
    logger.debug(f"Query list: {query_list}")
    if query_list:
        # Set maximum concurrent searches to avoid overwhelming the system
        MAX_CONCURRENT_SEARCHES = 2  # 设置最大并行数量
        
        # Use semaphore to limit concurrent searches
        semaphore = asyncio.Semaphore(MAX_CONCURRENT_SEARCHES)
        
        async def execute_search_with_semaphore(query: str) -> Dict[str, Any]:
            """Execute search with concurrency control"""
            async with semaphore:
                return await execute_single_search(query)
        
        logger.info(f"Starting parallel execution of {len(query_list)} searches with max concurrency: {MAX_CONCURRENT_SEARCHES}")
        search_results = await asyncio.gather(
            *[execute_search_with_semaphore(query_info) for query_info in query_list],
        ...
        for i, result in enumerate(search_results):
            if isinstance(result, Exception):
                ...
                logger.error(f"Exception in search {i}: {result}")
        ...
        search_results = []
        logger.warning("No optimized queries to execute")
    
    logger.debug(f"Processed search results: {len(processed_results)} results")
    ...
    logger.info(f"Search completed: {successful_searches}/{len(search_results)} successful")
    logger.info("Search execution node completed")
    
    return state

async def generate_followup_node(state: ResearchState, prompts: Dict[str, str], max_followup_iteration: int) -> ResearchState:
    """Node 3: Generate follow-up questions based on current findings."""
    logger.info("Starting follow-up question generation node")
    
    if state["research_iteration"] == max_followup_iteration:
        logger.info(f"Reached maximum follow-up iterations ({max_followup_iteration}), stopping research")
        state["should_continue"] = False
        return state

    logger.info(f"Generating follow-up questions for iteration {state.get('research_iteration', 1)}")
    ...
    logger.debug(f"Current findings count: {len(state.get('all_findings', []))}")
    ...
    try:
        ...
        logger.warning("Using fallback values for follow-up generation")
        # Fallback
        ...
        logger.info(f"Should continue research: {should_continue}")
        logger.info(f"Follow-up generation node completed, iteration incremented to {state['research_iteration']}")
...
async def generate_synthesis_node(state: ResearchState, prompts: Dict[str, str]) -> ResearchState:
    ...
    logger.info("Starting synthesis generation node")
    
    prompt = prompts.get('synthesis_generator_prompt', '')
    llm_provider: LLMProvider = llm_instance(model_name=os.getenv("DEV_MODEL_NAME"), system_prompt=prompt)
    
    logger.info(f"Generating synthesis for query: '{state['original_query']}'")
    ...
    logger.info(f"Synthesis context prepared with {len(state.get('all_findings', []))} findings")
    ...
    state["research_metadata"] = {
        ...
        "queries_used": len(state.get("optimized_queries", [])),
        "key_concepts": state.get("key_concepts", [])
    }
    
    logger.info("Synthesis generation node completed")
...
class DeepResearchOperator(BaseOperator):
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        async def _execute_search_wrapper(state: ResearchState) -> ResearchState:
            return await execute_search_node(state, self.prompts, self.system_prompt)
        
        async def _generate_followup_wrapper(state: ResearchState) -> ResearchState:
            return await generate_followup_node(state, self.prompts, self.max_iterations)
    ...
    async def arun(self, query: str, context: Optional[Dict[str, Any]] = None) -> OperatorResult:
        ...
        logger.info(f"🎯 Starting deep research for query: {query}, context: {context}")

        try:
            # Execute the research workflow
            final_state = await self.workflow.ainvoke(initial_state)
            logger.info(f"{self.name} Operator result: {final_state['final_synthesis']}")
