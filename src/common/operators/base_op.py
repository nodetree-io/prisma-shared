...
class OperatorResult(BaseModel):
    base_result: Any = Field(description="The main result content from the agent")
...
class BaseOperator(ABC):
    ...
    def inject_customize_prompt(self, system_prompt: str):
        """
        Inject a custom system prompt to override the default system prompt.
        
        This method allows dynamic customization of the operator's behavior by
        replacing the current system prompt with a user-provided one. This is
        useful for adapting the operator to specific tasks or domains without
        creating a new operator instance.
        
        Args:
            system_prompt: The custom system prompt to inject into the operator
        """
        self.system_prompt = system_prompt
